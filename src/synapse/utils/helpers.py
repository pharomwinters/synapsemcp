...
from synapse.database.base import SynapseDatabase
from mariadb import MariaDBDatabase
from sqlite_db import SQLiteDatabase
from duckdb_db import DuckDBDatabase
from synapse.core.config from synapse.core import config, ConfigurationError
...
def get_db_config_from_config() -> Dict[str, Any]:
    ...
    if db_type == "duckdb":
        ...
        db_config = {
            ...
            "port": config.get("database.mariadb.port", 3306),
            "user": config.get("database.mariadb.user", "root"),
            "password": config.get("database.mariadb.password", ""),
            "database": config.get("database.mariadb.database", "synapse")
        }
    else:
        # Fallback to DuckDB
        db_config = {
